...
class Board:
    def __init__(self):
        ...
        self.table.add_rows([[0, 0, 0],  # todo empty board
                             [0, 0, 0],
                             [0, 0, 0]], header=False)
    ...
    def empty_position(self, x, y):
        """
        Checks if the position [x,y] in the table is empty and move can be made there
        :param x: the row
        :param y: the column
        :return: if the position in the board is empty and a move can be made there
        """
        if self.table.rows[x][y] in ['X', 'O']:
            return False
        return True
    ...
    def empty_pos_left(self):
        ...
        for i in range(0, 3):
            for j in range(0, 3):
                if self.table.rows[i][j] != 'X' or self.table.rows[i][j] != 'O':
                    s = s + 1
        return s
