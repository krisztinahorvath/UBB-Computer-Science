...
class UndirectedGraph:
    ...
    def adj_matrix(self):
        g = [[0 for i in range(self.nr_vertices)] for j in range(self.nr_vertices)]
    ...
    def is_safe(v, graph, path, pos):
        ...
        :param path: path of the cycle
        :param pos: pos on which we want to insert the vertex
        :return:
        """
    ...
    def hamiltonian_cycle(self, graph, path, pos):
        """
        Recursive function to find if there is a hamiltonian cycle
        :param graph: the adjacency matrix of the graph
        :param path: the path of the cycle
        :param pos: positon on which we want to insert a vertex
        :return:
        """

        # if all positions are included in a hamiltonian cycle
        if pos == self.nr_vertices:
            if graph[path[pos - 1]][path[0]] == 1:
                return True
            return False

        for v in range(0, self.nr_vertices):
            # check if this vertex can be added to the hamiltonian cycle
            if self.is_safe(v, graph, path, pos):
                path[pos] = v
                # recur to construct rest of the path
                if self.hamiltonian_cycle(graph, path, pos + 1):
                    return True
    ...
    def find_hamiltonian_cycle(self):
        ...
        print(graph)
        path = [-1]*self.nr_vertices  # todo this works??
        ...
        p = self.hamiltonian_cycle(graph, path, 1)
        if p is False:
            print("\n\tThere is no hamiltonian cycle in the graph\n")
        else:
            print("\n\tThere is a hamiltonian cycle in the graph: ")
            for v in path:
                print(v, end=' ')
            print(path[0])
