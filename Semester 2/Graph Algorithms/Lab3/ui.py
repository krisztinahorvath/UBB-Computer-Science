from graph import Graph, read_file, write_file, random_graph, inf
...
class UI:
    ...
    def print_menu():
        print("Menu:\n"
              ...
              "\t23 - Lowest cost walk between two given vertices\n"
              "\t0 - Exit\n")
    ...
    def lowes_cost_walk_ui(self):
        x = int(input("x = "))
        y = int(input("y = "))

        try:
            cost, path = self._graph_list[self._this_graph].lowest_cost_walk(x, y)

            if cost == inf:
                print("No walk between ({}, {})".format(x, y))
            else:
                print("Lowest cost walk from ({}, {}) is {}".format(x, y, cost))
                print("Path: ", end="")
                if len(path) == 2:
                    print(path)
                else:
